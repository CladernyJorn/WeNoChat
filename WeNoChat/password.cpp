...
password::password(QString u, QTcpSocket *sock, QWidget *parent) : QWidget(parent),
                                                                   ui(new Ui::password)
{
    ui->setupUi(this);
    //去窗口边框
...
void password::hadreadyread()
{
    ...
    if (!reader.parse(find3back.toStdString(), jtmp))
    {
        qDebug() << "服务器返回信息错误\n";
        return;
    }
    ...
    std::string username;
    bool state;
    if (Decoder_findpWord3(jtmp["info"], username, state) == 0)
    {

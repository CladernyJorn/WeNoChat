...
user::user(QTcpSocket *sock, QWidget *parent) : QWidget(parent),
                                                ui(new Ui::user)
{
    ui->setupUi(this);
    client = sock;
...
void user::hadreadyread()
{
    ...
    if (!reader.parse(find1back.toStdString(), jtmp))
    {
        qDebug() << "服务器返回信息错误\n";
        return;
    }
    ...
    int secureQue;
    bool state;
    if (Decoder_findpWord1(jtmp["info"], username, state, secureQue, info) == 0)
    {

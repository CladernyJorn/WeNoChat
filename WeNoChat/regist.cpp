...
Regist::Regist(QTcpSocket *sock, QWidget *parent) : QWidget(parent),
                                                    ui(new Ui::Regist)
{
    ui->setupUi(this);
    //去窗口边框
...
void Regist::hadreadyread()
{
    ...
    if (!reader.parse(registback.toStdString(), jtmp))
    {
        qDebug() << "服务器返回信息错误\n";
        return;
    }
    ...
    bool state;
    if (Decoder_regist(jtmp["info"], username, info, state) == 0)
    {
